...
| `scene_selection` | Algorithm name + version (`temporal-stratified-lowest-cloud` / `1.0.0`), selected count, **every excluded item with its reason** | Selection changes results as much as processing does. Recording exclusions (`insufficient_aoi_overlap`, `cloud_cover_above_threshold`, `not_selected_temporal_sampling`) makes "why isn't scene X in my series?" answerable. |
| `processing` | Operation (`ndvi`), the full `ProcessingConfig` snapshot, masked SCL classes (ids and names) | The mask policy and every scientific parameter, frozen at run time. Config drift cannot silently change past results. |
| `software` | `processing_version` (1.0.0), git commit SHA, container image, Python version, `uv.lock` sha256, key package versions | Pins the exact code. Two runs with identical config but different rasterio/GDAL versions are not guaranteed bit-identical. |
| `scenes` | Per scene: item id, observation time, cloud cover, processing baseline, **band scaling used and its source**, original **unsigned** asset hrefs, usable flag + reason, output CRS/transform/resolution, processing seconds, warnings | The per-scene scientific record, including the baseline-offset decision (see the [offset pitfall](scientific-methodology.md#21-digital-numbers--reflectance-the-baseline-offset-pitfall)). |
...
```json
{
  ...
  "scene_selection": {
    ...
    "excluded": [
      {
        "item_id": "S2B_MSIL2A_20240612T162829_R083_T17TLH_20240612T221221",
        "reason": "not_selected_temporal_sampling"
      }
    ]
  },
  "processing": {
  ...
  "scenes": [
    {
      "item_id": "S2A_MSIL2A_20240607T163841_R126_T17TLH_20240608T014452",
